...
//http://stackoverflow.com/a/894929
...
function Dog(options) {
   if(options !== undefined) {
      this.color  = pick(options.color,undefined);
      this.hungry = pick(options.hungry, true);
      this.status = pick(options.status, 'normal');
      this.owner = pick(options.owner, undefined);
   }
   else {
      this.hungry = true;
      this.status = 'normal';
...
function Human(options) {
   if(options !== undefined) {
      this.cool  = pick(options.cool,false);
   }
   else {
      this.cool = false;
   }
